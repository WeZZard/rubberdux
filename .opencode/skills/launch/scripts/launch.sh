...
# Stop any leaked rubberdux VMs (only running instances, not base images)
for vm in $(tart list 2>/dev/null | grep '^local' | grep 'rubberdux-' | grep 'running' | awk '{print $2}' || true); do
    echo "Stopping leaked VM: $vm"
