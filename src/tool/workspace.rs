...
use crate::workspace::entity::{
    ProjectManifest, ProjectStatus, Responsibility, ResponsibilitiesDoc,
};
    ...
    fn execute<'a>(
        ...
        Box::pin(async move {
            ...
            match entity {
                ...
                "thread" | "artifact" | "resource" => ToolOutcome::Immediate {
...
impl WorkspaceTool {
    ...
    fn read_responsibilities_doc(&self) -> ResponsibilitiesDoc {
        let path = self.workspace.responsibilities_path();
        if !path.exists() {
...
mod tests {
    ...
    fn test_unimplemented_entity() {
        let (_root, tool) = temp_tool();
        let args = serde_json::json!({"entity": "thread", "action": "list"}).to_string();
        let (content, is_error) = assert_immediate(run(tool.execute(&args)));
        assert!(is_error);
