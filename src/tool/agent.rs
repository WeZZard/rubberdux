...
use crate::vm::rpc::{self, AgentToHost};
...
/// GeneralPurpose gets a full registry (no recursive `agent`).
pub fn build_subagent_registries(
    ...
    map.insert(SubagentType::GeneralPurpose, general_purpose);
    map
}
        ...
        Box::pin(async move {
            ...
            let subagent_type: SubagentType = match args.get("subagent_type") {
                Some(v) => match serde_json::from_value(v.clone()) {
                    ...
                    Err(_) => {
                        return ToolOutcome::Immediate {
                            content: "Invalid 'subagent_type'. Must be one of: explore, plan, general_purpose".into(),
                            is_error: true,
                        };
                    }
                },
            ...
            // Route: GP with RPC → VM spawn, otherwise in-process subagent
                ...
                && let Some(ref writer) = self.rpc_writer
            {
                ...
                log::info!("Requested VM agent {} from host", task_id);
                return ToolOutcome::Immediate {
                    content: format!(
                        "VM agent {} has been dispatched. The subagent is booting \
...
mod tests {
    ...
    #[tokio::test]
    async fn test_gp_no_rpc_returns_subagent() {
        let tool = dummy_agent_tool();
