...
- **The log is authoritative.** If a snapshot and the log ever disagree (a snapshot is corrupt,
  ...
  snapshot stores `digest = Fingerprint(hex(SHA-256(serde_json::to_vec(&World))))` (computed by
  `snapshot.rs::world_digest`, reusing the same `Fingerprint` encoding as `effects::fingerprint_call`
  — Inv 7); on restore the loader recomputes the digest of the deserialized World and REJECTS the
  snapshot if it does not match (corruption / bit-flip), falling back to an EARLIER snapshot (or
  genesis) + tail replay. Because the log remains authoritative, a rejected snapshot NEVER causes
  data loss — the World is simply rebuilt from the log.
- **Pruning rule.** Retain only the most recent **K** snapshots; older snapshots are deleted because
  they are REGENERABLE by replay from an earlier snapshot (or genesis) plus the log tail. The
  **log itself is never pruned below what is needed to regenerate** a retained snapshot's successors —
  cold-storing fully-covered segments is allowed, deleting them is not (that would destroy the source
  of truth). Pinned defaults (`Caps::default()` in `world.rs`): `snapshot_interval = 64` ticks,
  `snapshot_keep (K) = 3`; `0` in either field means **unbounded** (no periodic snapshots / no
  eviction). Logs serialized before `Caps` was introduced deserialize to these defaults via
  `#[serde(default)]` on `Caps`, so existing sessions are unaffected.
...
## Branch identity and storage layout

A **branch** is a counterfactual session rooted at a fork point on the recorded (MAIN) log or on
another branch. Branches share a common prefix with their parent; only the divergent tail — the
events from the fork tick onward — is branch-local.

### Content-addressed BranchId

`BranchId` (`branch.rs`) is the content address of a fork:

```
BranchId = hex(SHA-256(parent_ref ‖ fork_tick.to_le_bytes() ‖ canonical(edit)))
```

where:
- `parent_ref` = `parent.0.as_bytes()` — the parent `BranchId`'s string bytes (empty for MAIN).
- `fork_tick.to_le_bytes()` — the fork tick as a little-endian `u64`.
- `canonical(edit)` = `serde_json::to_vec(&edit)` — the canonical JSON serialization of the
  [`Edit`](../../../src/agent/world/branch.rs) struct (field order fixed by `serde`).

The three byte sequences are fed to `SHA-256` in the fixed order above and the digest is encoded
as `hex::encode(...)` — the same encoding `effects::fingerprint_call` uses (Inv 7). The same
parent, fork tick, and edit always produce the same `BranchId`; distinct edits produce distinct ids
(SHA-256 collision-resistant).

**`BranchId::MAIN`** is the reserved constant (the empty string) that identifies the root recorded
log. Every computed `BranchId` is a 64-char hex digest, so the empty string is unambiguous as a
sentinel and is uncomputable by `BranchId::compute`.

### Exogenous-edit scope

Only **exogenous** inputs (the free variables of the trajectory) may be the target of an `Edit`.
The validator `edit_is_exogenous` (`branch.rs`) delegates to `replay::is_exogenous`, the exact
complement of `effects::is_derived_result`: derived effect-results — `ModelResponded`,
`ModelFailed`, `InferenceCancelled`, `ToolReturned`, `HumanActionDone`, `Compacted` — are
**rejected**; every other input (user intents, peer traffic, surface events, `SessionStarted`, …)
...
Branch artifacts live under the session directory alongside the main log:
...
## Dead-branch pruning and reachability GC

When a counterfactual branch is abandoned it accumulates on disk indefinitely unless the runtime
can reclaim it. The dead-branch GC (`src/agent/world/reclaim.rs`) is designed around a single
safety constraint: **nothing reachable from a root is ever removed** (Invariant 20). It realises
this through the same pure-policy / thin-IO-shell split used elsewhere in the runtime.

### Root set

A branch is **reachable** if it belongs to any of five root categories:

1. **Live session.** `Roots.live_session` (typically `BranchId::MAIN`, the recorded MAIN log) is
   always a root. The shared parent prefix it owns is therefore never touched.
2. **Pinned.** A branch whose `BranchDescriptor.pin` flag is `true`, or whose id appears in
   `Roots.pinned`, is a root. `pin` (`reclaim.rs`) persists this state append-only (see below).
3. **Live handle.** A branch whose id appears in `Roots.live_handles` (an in-memory set tracking
   open replay sessions) is a root.
4. **Parent of a retained branch.** The root set is closed UPWARD through parents: for each
   reachable branch its parent — and the parent's parent, transitively — is also reachable. This
   preserves the shared lineage so no divergent prefix is silently deleted.
5. **Within `Caps.branch_grace`.** A branch younger than `branch_grace` ticks is retained by the
   time-dependent grace window. `grace == 0` disables age-based reclamation entirely (unbounded
   grace); in that case only an explicit drop makes a branch dead.

### Pure policy: `reachable` and `reclaimable`

`reachable(branches, roots) -> BTreeSet<BranchId>` (`reclaim.rs`) is a **pure, time-independent**
function. It seeds the root set from categories 1–4 above and then closes it upward through the
`BranchDescriptor.parent` chain, terminating at an id absent from the branch list (e.g. `MAIN`,
the root log) and guarding against any malformed cycle. It calls no clock.

`reclaimable(branches, roots, now) -> Vec<BranchId>` is also pure. A branch is reclaimable iff it
is BOTH (1) not in `reachable(…)` and (2) **dead**: either `BranchDescriptor.tombstone == true`
(an explicit drop) or aged past the grace window (`grace != 0` and `now - created_wall >= grace`).
`now` is passed in; no ambient `now()` is called.

### Append-only persistence: `pin` and `drop`

`pin(session_dir, id, pinned)` and `drop_branch(session_dir, id)` are the only reachability-state
mutators. Both are **append-only latest-line-wins**: a fresh `BranchDescriptor` with the updated
`pin` or `tombstone` field is appended to `branches/index.jsonl`. `load_latest_descriptors`
collapses the index by id, keeping the LAST line for each — exactly the JSONL discipline the rest
of the log uses (no in-place rewrite, no truncation).

### Sweep: the thin IO shell

`sweep(session_dir, dead)` deletes each dead branch's OWN artifacts:
`branches/<id>/world-events.jsonl` and `branches/<id>/snapshots/` — i.e. the whole
`branches/<id>/` directory. It **never** touches:
- a reachable branch (the pure policy excludes it before the shell is called);
- the shared parent prefix (`<session_dir>/world-events.jsonl`, the MAIN log — outside
  `branches/`);
- `BranchId::MAIN` — the empty-string sentinel resolves to `branches/` itself, so the shell
  guards that case defensively even though `reclaimable` can never include it.

`reclaim_branches(session_dir, roots, now)` is the composed seam: load descriptors →
`reclaimable` → `sweep`. Returns the ids whose directories were removed (idempotent: an
already-swept branch is silently skipped).

---

## Segmented log and cold-storage tiering

The logical event log (`src/agent/world/segment.rs`) is append-only and is the single source of
truth (Invariant 9). As sessions age, fully-replayed segments become redundant for recovery but
must not be deleted — the log is **never pruned**. The segmented log bounds the hot working set by
**tiering** (moving, never deleting) covered segments to a `cold/` subdirectory.

### Segment model: `SegmentedEventLog` and `Manifest`

`SegmentedEventLog` wraps the `EventLog` trait. Its stratum-1 stream is split into tick-contiguous
**segments** described by a `manifest.json` in the session directory. Each `SegmentEntry` records
`{ id: SegmentId, first_tick, last_tick, tier: Hot | Cold }`.

- **Seal-and-roll.** `seal_and_roll(at_tick)` freezes the open segment's real `first_tick` /
  `last_tick` (recomputed from the segment's actual events) and opens a fresh Hot segment named
  `seg-<at_tick>`. It is called at snapshot boundaries so the floor from `retention.rs` aligns
  with segment boundaries. The manifest is persisted atomically (temp + fsync + rename) after each
  roll. Sealing an empty segment or one whose `first_tick` is not strictly before `at_tick` is a
  no-op (no empty-segment churn).
- **Segment ids.** The first (and, before the first roll, only) segment carries the M1-compatible
  id `world-events` — the same file stem an M1 single-file session uses, so a pre-manifest session
  is byte-compatible with the new format. Subsequent segments carry `seg-<first_tick>`.
- **Open (active) segment.** The LAST entry in `manifest.segments` is always the open segment;
  all appends land there. It is mutable and is never tiered.

### Pure policy: `cold_storable`

`cold_storable(manifest, floor) -> Vec<SegmentId>` is a **pure** function over the manifest and a
protection `floor` (the `first_tick` of the oldest retained snapshot from `retention.rs`). A
segment is eligible iff it is:
1. **Not the open (active) segment** — the last entry is mutable; only sealed segments move.
2. **Still `Hot`** — already-`Cold` segments are skipped (idempotence).
3. **Fully covered** — `last_tick < floor` (strict less-than). Because `floor` is the first tick
   of the oldest retained snapshot, any segment ending before it can be regenerated from that
   snapshot plus the hot tail; moving it loses nothing.

A `floor` of `0` protects every segment, matching the "no retained snapshot → tier nothing" case.

### Tiering: `tier_to_cold` and `tier_cold_storable`

`tier_to_cold(id)` **moves** (never deletes) a segment file from the hot directory
(`<session_dir>/<id>.jsonl`) to `cold/` (`<session_dir>/cold/<id>.jsonl`). The manifest is
updated to `Tier::Cold` **only after** the bytes are durably in `cold/`, so a crash cannot leave
the manifest pointing at an absent file. `load` resolves either tier defensively regardless.

`tier_cold_storable(floor)` is the thin IO composition: `cold_storable` (pure plan) →
`tier_to_cold` per selected segment. Returns the ids moved.

### `load` is byte-identical before and after tiering

`SegmentedEventLog::load` sorts segments by `first_tick` (cold/oldest first) and concatenates each
segment's events in order. Because tiering moves bytes without altering them, and `load` reads from
whichever tier the segment currently lives in, the returned sequence is **byte-identical** before
and after any number of tiering passes.

### M1 backward compatibility

A session directory with no `manifest.json` (a pre-M1 or fresh segmented log before its first
roll) is opened as ONE implicit Hot segment named `world-events`. Opening a manifest-less log is
**side-effect-free**: no `manifest.json` is written, and a subsequent `open` loads the same
implicit single-segment manifest. M1 sessions load unchanged.
...
20. **Reclamation deletes only regenerable or unreachable artifacts; the log is tiered, never
    deleted; nothing reachable from a root is ever removed.** Snapshot pruning (`retention.rs`)
